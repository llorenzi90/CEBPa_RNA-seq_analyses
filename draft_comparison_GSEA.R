...
for (geneset in c("Hallmarks","c2","c3","c7")) {
  ...
  for(qval in c(0.05,0.01)){
    ...
    pdf(paste0("test_",geneset,"_filtered_min_qval_",qval,".pdf"))
    print(g)
    dev.off()
  ...
  for(qval in c(0.05,0.01)){
    ...
    pdf(paste0("test_",geneset,"_filtered_min_qval_",qval,".sortedbyNES.pdf"))
    print(g)
    dev.off()
...
abbreviate(str_to_title(gsub("HALLMARK.","",gsub("_",".",data_all_filtered$NAME))),minlength = 15)
